...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  + 각 다리 라벨 옆 화살표 `#bfTrend`/`#hgTrend`(↑/↓/↔ + Δcm). **판정 = 수위 추세 + 인천 조위 위상 교차검증**.
  - `wlTrend(rows)`: 각 다리 최근 `FLOW.winMin`(30분) 변화량 Δcm → up/down/flat(`deadCm`=2). **추세라 방류량 baseline 자동 상쇄**
    (예전 '절대 임계값 불안정' 문제 회피). 두 관측소 gdt 달라 절대값 상호비교는 안 함.
  - `tidePhaseNow(list)`: 인천 오늘 고저조에서 지금 이후 첫 극값이 고조=밀물(flood)/저조=썰물(ebb).
  - `renderFlow()`: 전역 `flow={bf,hg,tide}`(각 fetch가 저장). **밀물&행주↑→역류 경향**(한강대교↑면 '여의도까지'),
    **썰물&행주↓→순류**, 상충·정체·위상 unknown→정체·전환점(경향형, 무리한 단정 안 함). 수동 모드는 배지 off.
  - 재사용: `renderWL`이 추세 계산·저장, `renderTide` 끝에서 위상 저장. **새 API·호출 0**(기존 데이터 재활용).
- **한계·후속(문구는 '경향'으로)**: 한강대교 조석 반응 실측 미확인 → 실제 역류 사례로 `winMin`/`deadCm` 보정 필요.
  밀물이어도 인천 조위<신곡수중보(≈720cm)면 실제 역류 아닐 수 있음 → 필요 시 720 게이팅을 후속 추가(1019635 아라한강갑문으로 신곡보 수위 조회 가능).
