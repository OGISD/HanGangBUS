...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  ...
  - **`leadLag(bfRows,hgRows)`: 누가 먼저 움직였나(2026-07-27 추가).** 1차 차분(움직임)을 10분 격자로 **교차상관**
    ...
    일반 화면에서는 기술 용어 `선행`을 쓰지 않고 `행주 N분 먼저 변함(조석 추정)` 또는
    `한강대교 N분 먼저 변함(방류 추정)`으로 표시한다. `.flow-nowrap`으로 이 짧은 문구 내부 줄바꿈을 막는다.
  ...
  - `flowVerdict(bfDir,hgDir,ph)` / `renderFlow()`: 전역 `flow={bf,hg,bfRows,hgRows,tide}`를 사용해 방향을 대칭 판정.
    ...
    `상류 흐름 가능성`/`하류 흐름 가능성`으로 낮춰 표시한다. 다른 관측소가 반대 방향이면 `지점별 흐름 차이`를 근거줄에 추가.
    정확히 반대 입력은 반대 결과가 나오도록 같은 `phaseDir`·`matches` 계산을 공유한다.
    두 관측소 모두 임계 안쪽일 때만 `수위 변화 작음`, 물때와 움직임이 맞지 않는 나머지는
    `방향 엇갈림·판정 불확실`로 표시한다. 수위 변화 작음은 실제 물의 유속이 0이라는 뜻이 아니다.
  ...
  - **신선도와 추세 구간은 별개**: `STALE_MIN=30`은 현재 판정에 사용할 수 있는 최신 관측의 최대 나이다.
    ...
    같은 배지에 `수위 자료 오래됨 · 역류 판정 중단`을 표시한다. `FLOW.winMin=30`은 신선한 최신 관측을 기준으로
