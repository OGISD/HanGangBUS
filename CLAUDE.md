...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  ...
  - **신선도와 추세 구간은 별개**: 현재 `STALE_MIN=30`은 관측시각을 빨간색으로 경고할 뿐 `renderFlow()` 판정을
    중단하지 않는다. `FLOW.winMin=40`은 최신 관측을 기준으로 기울기에 포함할 과거 범위다. 2026-08-03 실사용 확인 후
    추세를 60분으로 늘리고 최신 관측이 30분 이상 오래되면 판정을 중단하는 안을 논의 중이며, 승인 전 변경 금지.
