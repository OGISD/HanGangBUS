...
- 판정 3단계: pass<noGo→운항 금지(빨강), pass<warn→운항 경고(주황), 그 외 여유(초록).
  **warn≤noGo이면 경고 단계 자체가 안 뜸**(행주대교가 그 경우 — 여유/금지 2단계).
...
- 저장 키 `hb_bridges5`(2026-07-06 bump — 새 임계값 강제 적용). 예전 `hb_bridges4`는 폐기.
...
- **방류량 절대값 운항금지 판정**: 각 열의 최신값(강조 행)을 `DAM.noGoFlow`(=3000㎥/s, 회사 확인값)와
  비교. 넘으면 그 열 아래 빨간 칩 표시 — 4시간 전 열은 "여의도 운항금지"(지금 여의도),
