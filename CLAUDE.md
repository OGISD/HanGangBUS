...
- Codex가 실제 변경에 참여한 커밋은 `Co-authored-by: Codex <codex@openai.com>`으로 GitHub 공식 `codex` 계정에 연결한다.

## 프로젝트 한 줄 요약
**한강버스 운항 도우미** (앱 표시 이름. 제작자 표기 `오동근, Claude·GPT와 함께`, 상세 역할은 `CONTRIBUTORS.md`).
...
## 조류세기 (인천 카드 하단) — KHOA 수치조류도 예측
- **KHOA 수치조류도 예측 유향·유속** API로 계산. 표시는 **오늘 하루 물살 세기 %**(하루 고정,
  방향 화살표 없음, 깔끔한 %). 예: 오늘 59%(바다타임 일치). (조차 추정[방법 A]·실시간 유속 방식은 폐기.)
- 계산: 하루 여러 시각(`TIDECUR.hours`=0,3,…,21)의 인천 최근접 격자 유속 중 **최고값** = 오늘 하루 세기.
  이를 `TIDECUR.maxSpeed`(125cm/s, 오늘 최고 74→59% 보정)로 나눠 %. 절대적인 운항 안전 비율은 아니며 예측 참고값이다.
...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  + 각 다리 라벨 옆 실제 1시간 변화 `#bfTrend`/`#hgTrend`(↑/↓/↔ + cm/1h). **판정 = 수위 추세 + 여의도 추정 조위 위상 + 두 다리 시간차**.
  - `wlTrend(rows)`: 최근 `FLOW.winMin`(현재 30분)의 **최소제곱 기울기 → cm/h**. 2점 차분이 끝값 하나에
    흔들리던 문제 해소. 판정 `deadRate`=±2cm/h. 시각은 `ymdhmToMs`로 계산해 **결측(빈 wl)이 섞여도 안전**.
    **추세라 방류량 baseline 자동 상쇄**(예전 '절대 임계값 불안정' 회피). 두 관측소 gdt 달라 절대값 상호비교는 안 함.
    민감도 핵심값은 `FLOW_TUNING={winMin:30, deadRate:2}` 한곳에서만 관리하며 `FLOW`가 이를 참조한다.
    도움말의 계산 구간·임계값도 `syncFlowTuningText()`가 자동 반영하므로 조정할 때 HTML 문구를 따로 고치지 않는다.
  - `hourlyChange(rows)`: 최신 관측 수위에서 **정확히 60분 전 관측 수위**를 뺀 실제 변화량(cm).
    기존처럼 30분 추세 기울기를 시간당으로 환산하지 않는다. 정확한 시각의 관측이 없으면 보간하거나 가까운 값을
    대신 쓰지 않고 `null`을 반환해 라벨 숫자를 숨긴다. 이 표시는 이해를 돕는 별도 정보이며 `wlTrend()` 판정에는 쓰지 않는다.
  - `tidePhaseNow(list,shiftMin,nowKstMs)`: 인천 고저조를 `TIDE.lagMin`(약 4시간) 늦춰 여의도 추정 위상을 계산.
    자정 직후에도 올바르게 계산하도록 내장 연간표의 전날 고저조를 함께 사용한다.
  - **`leadLag(bfRows,hgRows)`: 누가 먼저 움직였나(2026-07-27 추가).** 1차 차분(움직임)을 10분 격자로 **교차상관**
    ...
    일반 화면에서는 기술 용어 `선행`을 쓰지 않고 `행주 수위 N분 먼저 변함 · 조석 영향 추정` 또는
    `한강대교 수위 N분 먼저 변함 · 방류 영향 추정`으로 표시한다. `.flow-nowrap`으로 이 짧은 문구 내부 줄바꿈을 막는다.
    ⚠ **용어 주의(2026-07-27)**: 원래 "조석 서명/상류 서명"(signature 직역)이라 썼다가 **일반 사용자에게 이질적**이라
    교체함. 원인은 시간차로 미루어 짐작하는 것이라 **반드시 "추정"을 붙일 것**(확정처럼 쓰지 말 것).
  - `flowVerdict(bfDir,hgDir,ph)` / `renderFlow()`: 전역 `flow={bf,hg,bfRows,hgRows,tide}`를 사용해 방향을 대칭 판정.
    밀물+상승 또는 썰물+하강이 한 관측소 이상에서 맞으면 일치 개수와 관계없이 실제 방향을 단정하지 않는
    `상류로 흐르는 경향`/`하류로 흐르는 경향`으로 표시한다. 다른 관측소가 반대 방향이면 `두 지점의 수위 변화가 다름`을 근거줄에 추가.
    정확히 반대 입력은 반대 결과가 나오도록 같은 `phaseDir`·`matches` 계산을 공유한다.
    두 관측소 모두 임계 안쪽일 때만 `수위 변화 작음`으로 표시한다. 여의도 추정 물때와 두 관측소 수위 방향이 맞지 않으면
    `흐름 방향 판단 어려움`으로 표시한다. 물때가 없을 때도 수위 변화만으로 실제 흐름 방향을 강제하지 않는다.
    수위 변화 작음은 실제 물의 유속이 0이라는 뜻이 아니다.
    시간차는 근거줄에 항상 표기하고, **역류 판정인데 한강대교가 선행이면 '상류(방류) 영향 가능'을 덧붙여 단정을 낮춤**.
    수동 모드는 배지 off(`flow.*Rows`도 비움).
  - 재사용: `renderWL`이 추세 계산·시계열 저장, `renderTide` 끝에서 여의도 추정 위상 저장. 전날 고저조는 같은 저장소의
    연간표에서 보충하며 별도 공공 API는 호출하지 않는다.
  - **관리자 진단줄 `#flowDiag`(2026-07-27 추가, `setFlowDiag()`)**: 판정에 쓴 원시 수치를 그대로 노출 —
    `진단 · 행주 -10cm/h(30분·4점) · 한강대교 +2cm/h(...) · 임계 ±2cm/h · 시간차 한강대교 50분 먼저 · 상관 0.66 · 진폭 15cm · 13쌍 · 위상 썰물`.
  ...
  - **신선도와 추세 구간은 별개**: `STALE_MIN=30`은 현재 판정에 사용할 수 있는 최신 관측의 최대 나이다.
    행주·한강대교 중 하나라도 최신 수위가 30분 이상 오래됐거나 없으면 `renderFlow()`가 판정과 추세 숫자를 숨기고
    같은 배지에 `수위 자료가 오래되어 판정 보류`를 표시한다. `FLOW.winMin=30`은 신선한 최신 관측을 기준으로
    기울기에 포함할 과거 범위이며, 카드의 4시간 기록과 시간차 교차상관 자료는 그대로 유지한다. 60분 구간은
    2026-08-04 실사용에서 40분으로 복원했고, 2026-08-05 약 44시간 자료 비교에서는 40분도 전환 직후 변화를 늦추는
    사례가 확인되어 30분으로 단축했다. 당시 오늘 자료 기준 움직임 판정은 73.9%→81.1%였으며, 실제 조류 실측이
    없는 민감도 비교이므로 당분간 현장 체감에 따라 `FLOW_TUNING`만 조정하고 변경 이력을 남긴다.
