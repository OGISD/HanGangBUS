...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  + 각 다리 라벨 옆 실제 1시간 변화 `#bfTrend`/`#hgTrend`(↑/↓/↔ + cm/1h). **판정 = 수위 추세 + 인천 조위 위상 + 두 다리 시간차**.
  ...
  - `flowVerdict(bfDir,hgDir,ph)` / `renderFlow()`: 전역 `flow={bf,hg,bfRows,hgRows,tide}`를 사용해 방향을 대칭 판정.
    밀물+상승 또는 썰물+하강이 두 관측소에서 모두 맞으면 기존 역류 경향/순류, 한쪽만 맞으면
    `상류 흐름 가능성`/`하류 흐름 가능성`으로 낮춰 표시한다. 다른 관측소가 반대 방향이면 `두 지점의 수위 변화가 다름`을 근거줄에 추가.
    정확히 반대 입력은 반대 결과가 나오도록 같은 `phaseDir`·`matches` 계산을 공유한다.
    두 관측소 모두 임계 안쪽일 때만 `수위 변화 작음`으로 표시한다. 물때와 두 관측소 수위 방향이 모두 반대여도
    판정을 멈추지 않고 물때 기준 `상류/하류 방향 예상`을 표시하며, 근거줄에 `수위 변화와 맞지 않아 신뢰 낮음`을 추가한다.
    물때 정보가 없고 두 관측소 방향도 서로 다를 때만 `흐름 방향 판단 어려움`으로 표시한다.
