...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  ...
  - `wlTrend(rows)`: 최근 `FLOW.winMin`(40분)의 **최소제곱 기울기 → cm/h**. 2점 차분이 끝값 하나에
  ...
  - **신선도와 추세 구간은 별개**: `STALE_MIN=30`은 현재 판정에 사용할 수 있는 최신 관측의 최대 나이다.
    ...
    같은 배지에 `수위 자료 오래됨 · 역류 판정 중단`을 표시한다. `FLOW.winMin=40`은 신선한 최신 관측을 기준으로
    기울기에 포함할 과거 범위이며, 카드의 4시간 기록과 시간차 교차상관 자료는 그대로 유지한다. 60분 구간은
    2026-08-04 실사용 및 하루치 276개 판정 비교에서 흐름 전환을 늦추는 사례가 반복되어 40분으로 복원했다.
