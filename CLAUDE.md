...
- **역류/순류 판정 v1 구현(2026-07-15)**: 카드 상단 종합 배지 `#flowBadge`(🔺역류 경향/🔻순류/⏸정체·전환점 + 근거줄)
  ...
  - **`leadLag(bfRows,hgRows)`: 누가 먼저 움직였나(2026-07-27 추가).** 1차 차분(움직임)을 10분 격자로 **교차상관**
    (`corrMin` 3시간 구간, `maxLagMin` ±60분 탐색). **양수 lag = 행주 선행 = 조석(역류) 서명**, 음수 = 한강대교 선행 =
    **상류(팔당댐 방류) 서명**. 차분이라 gdt·기준선 차이 자동 상쇄. 진폭 `minAmpCm`(5cm) 미만이거나 상관 `minCorr`(0.5)
    미만이면 `lead='unknown'`(노이즈 보류). ⚠ 부호 규칙은 합성 데이터로 검증함(행주 30분 선행 → lagMin +30, corr 1.0).
    실측 예: 2026-07-27 행주 선행 40분·상관 0.85 — 조석이 하류→상류로 전파되는 실제 서명이 잡힘.
...
- **한계·후속(문구는 '경향'으로)**: 실제 역류 사례로 `winMin`/`deadRate`/`minCorr` 보정 필요.
  ...
  - **팔당댐 방류량 보정도 보류** — 영향은 존재하나 그 크기·도달 범위를 정량화할 근거가 없음(사용자 판단).
    대신 `leadLag`의 '상류 서명' 감지가 같은 문제를 정성적으로 잡아줌.
